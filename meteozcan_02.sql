...
--
-- Anamakine: 127.0.0.1
-- Üretim Zamanı: 01 Ara 2020, 14:29:19
...
(8, '2020-11-03 20:36:20', 'İletişim', 'https://iletisim.com.tr', 'iletisim@iletisim.com.tr'),
(9, '2020-12-01 11:44:59', 'Sinek Sekiz', 'www.sineksekiz.com\r\n', 'info@sineksekiz.com'),
(10, '2020-12-01 11:46:09', 'Remzi Kitabevi', 'http://www.remzi.com.tr/anasayfa\r\n', 'post@remzi.com.tr');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `liste`  AS SELECT `k`.`KitapAdi` AS `kitapAdi`, concat(`r`.`YazarAdi`,' ',`r`.`YazarSoyadi`) AS `AdSoyad`, `y`.`YayinEviAdi` AS `YayinEviAdi`, `k`.`KitapBasimYeri` AS `KitapBasimYeri`, `k`.`KitapDili` AS `KitapDili`, `k`.`KitapSayfaSayisi` AS `KitapSayfaSayisi` FROM ((`kitap` `k` join `yayinevi` `y`) join `yazar` `r`) WHERE `k`.`KitapKayitNo` = `y`.`YayinEviKayıtNO` AND `k`.`KitapKayitNo` = `r`.`YazarKayıtNo` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `url`  AS SELECT `k`.`KitapISBN` AS `KitapISBN`, `y`.`YayinEviURL` AS `YayinEviURL`, `r`.`YazarURL` AS `YazarURL` FROM ((`kitap` `k` join `yayinevi` `y`) join `yazar` `r`) WHERE `k`.`KitapKayitNo` = `y`.`YayinEviKayıtNO` AND `k`.`KitapKayitNo` = `r`.`YazarKayıtNo` ;
